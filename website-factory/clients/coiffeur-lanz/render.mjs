...
function scene(content) {
  ...
  const responsive = asset === '/assets/salon-stilllife.webp' ? ' srcset="/assets/salon-stilllife-small.webp 560w, /assets/salon-stilllife.webp 1122w" sizes="(max-width: 760px) 100vw, (max-width: 1440px) 42vw, 650px"' : '';
  return `<figure class="scene-wrap"><div class="scene"><div class="scene-rings" aria-hidden="true"></div>${asset ? `<img src="${esc(asset)}"${responsive} alt="${esc(content.heroAlt)}" fetchpriority="high" width="1122" height="1402">` : ''}<span class="scene-word" aria-hidden="true">Lanz</span></div>${asset === '/assets/salon-stilllife.webp' ? '<figcaption>Symbolbild · keine Aufnahme des Salons</figcaption>' : ''}</figure>`;
}
...
function home(content) {
  ...
  return `<section class="hero section-wrap"><div class="hero-copy"><p class="eyebrow">${esc(content.home.eyebrow)}</p><h1>${esc(content.home.headline)}</h1><p class="lead">${esc(content.home.intro)}</p><div class="hero-actions">${link('/kontakt/', 'Termin anfragen', 'button button-solid')}<a class="quiet-link" href="${esc(href(b.phone))}">Oder anrufen: ${esc(b.phone)}</a></div></div>${scene(content)}</section>
  <section class="intro-section section-wrap" aria-labelledby="intro-title"><p class="section-kicker">Persönlich in Bleienbach</p><div><h2 id="intro-title">Bei Vreni Lanz.</h2><p>Coiffeur Lanz finden Sie an der Eichi 20 in Bleienbach. Für Termine und Fragen ist Vreni Lanz Ihre direkte Ansprechperson.</p>${link('/salon/', 'Mehr über den Salon')}</div></section>
  <section class="visit-preview section-wrap" aria-labelledby="visit-title"><div><h2 id="visit-title">Ihr Weg zu uns.</h2><p>${esc(addressText(b))}</p>${link('/besuch/', 'Anfahrt und Öffnungszeiten')}</div><div class="preview-hours"><h3>Öffnungszeiten</h3>${hours(content)}</div></section>${notice(content)}${contactShort(b)}`;
}

function salon(content) {
  const b = content.business;
  return `<section class="page-intro section-wrap salon-intro"><div><p class="eyebrow">Der Salon</p><h1>${esc(content.salon.headline)}</h1><p class="lead">${esc(content.salon.intro)}</p></div>${scene(content)}</section>
  <section class="detail-section section-wrap"><h2>Vreni Lanz</h2><div><p>Ein persönlicher Termin lässt Raum für Ihre Fragen. Nehmen Sie direkt Kontakt auf, wenn Sie einen Besuch planen oder etwas klären möchten.</p>${link('/kontakt/', 'Kontakt aufnehmen')}</div></section>
  <section class="place-strip section-wrap"><p>Der Standort</p><h2>Eichi 20<br>3368 Bleienbach</h2>${link('/besuch/', 'Besuch planen')}</section>${contactShort(b)}`;
}
...
function visit(content) {
  ...
  <section class="detail-section section-wrap"><h2>Vor Ihrem Besuch</h2><div><p>Rufen Sie für eine Terminabsprache an. Eine Nachricht über das Kontaktformular ist eine Anfrage; der Termin steht erst nach einer persönlichen Bestätigung fest.</p><a class="text-link" href="${esc(href(b.phone))}">${esc(b.phone)} <span aria-hidden="true">↗</span></a></div></section>${contactShort(b, 'Bis bald in Bleienbach.')}`;
}
...
function journal(content) {
  ...
  return `<section class="page-intro section-wrap narrow"><p class="eyebrow">Journal</p><h1>Aus dem Salon.</h1><p class="lead">Neuigkeiten und Gedanken von Coiffeur Lanz finden Sie hier, sobald sie veröffentlicht sind.</p></section>
...
function missing() {
  return `<section class="page-intro section-wrap narrow"><p class="eyebrow">404</p><h1>Seite nicht gefunden.</h1><p class="lead">Dieser Weg führt hier nicht weiter. Über die Startseite finden Sie zurück zu Coiffeur Lanz.</p>${link('/', 'Zur Startseite', 'button button-solid')}</section>`;
}
...
export function renderPage(pathname, content, options = {}) {
  ...
  const [title, body] = pages[path] || (article ? [article.title, articlePage(article)] : ['Seite nicht gefunden', missing()]);
