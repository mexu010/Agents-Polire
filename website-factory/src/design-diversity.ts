/**
 * Pure helper from the supplied design-upgrade kit. Factory adaptation lives
 * in design-policy.ts; this module has no database, model, browser or network access.
...
function integer(
  ...
  if (
    ...
    return fail(
      `${label}: expected an integer between ${minimum} and ${maximum}`,
    );
  }
  return value;
}
function same(
  left: DesignSignature,
  right: DesignSignature,
  feature: Feature,
): boolean {
  if (feature !== "section_order")
    return left.features[feature] === right.features[feature];
...
function sameCatalog(left: DesignSignature, right: DesignSignature): void {
  if (
    left.version !== 2 ||
    right.version !== 2 ||
    left.capabilities_version !== right.capabilities_version
  ) {
    fail(
      "Incompatible signature versions; migrate from verified plans or report a comparison gap",
    );
  }
}
...
export function parseCatalog(raw: unknown): CapabilityCatalog {
  ...
  for (const feature of SCALAR_FEATURES)
    features[feature] = tokens(entries[feature], `catalog.${feature}`);
  return {
    version: id(root.version, "catalog.version"),
    features,
    components: tokens(root.components, "catalog.components"),
  };
}
...
export function parseSignature(
  ...
  if (root.capabilities_version !== catalog.version)
    fail("signature: capability version mismatch");
  ...
  for (const feature of SCALAR_FEATURES) {
    ...
    if (!catalog.features[feature].includes(value))
      fail(`signature.${feature}: unsupported capability ${value}`);
    features[feature] = value;
  }
  const order = tokens(entries.section_order, "signature.section_order", true);
  for (const component of order) {
    if (!catalog.components.includes(component))
      fail(`signature.section_order: unsupported component ${component}`);
  }
...
export function compareSignatures(
  ...
  const weight = (items: readonly Feature[]) =>
    items.reduce((sum, feature) => sum + WEIGHTS[feature], 0);
  return {
    matched,
    different,
    structural_differences: different.filter((feature) =>
      STRUCTURAL_FEATURES.includes(feature),
    ),
    declared_similarity: weight(matched) / weight(FEATURES),
    same_composition_and_order:
      same(left, right, "composition") && same(left, right, "section_order"),
    interpretation: "declared_features_not_visual_quality",
  };
}
...
export function parseConcepts(
  ...
  if (!Array.isArray(raw) || raw.length < 2 || raw.length > 6)
    fail("concepts: expected 2 to 6 concepts");
  ...
  return raw.map((entry: unknown) => {
    ...
    return {
      ...
      signature: parseSignature(item.signature, catalog),
    };
  });
}
...
export function assessConceptSet(
  ...
  options: {
    minimum_differences?: number;
    minimum_structural_differences?: number;
  } = {},
) {
  integer(concepts.length, 2, 6, "concept count");
  const minimum = integer(
    options.minimum_differences ?? 4,
    1,
    FEATURES.length,
    "minimum_differences",
  );
  const structuralMinimum = integer(
    options.minimum_structural_differences ?? 2,
    1,
    STRUCTURAL_FEATURES.length,
    "minimum_structural_differences",
  );
  const comparisons: Array<
    Comparison & { left: string; right: string; distinct_enough: boolean }
  > = [];
  for (let a = 0; a < concepts.length; a += 1) {
    for (let b = a + 1; b < concepts.length; b += 1) {
      ...
      comparisons.push({
        left: left.concept_key,
        right: right.concept_key,
        ...comparison,
        distinct_enough:
          comparison.different.length >= minimum &&
          comparison.structural_differences.length >= structuralMinimum,
      });
    }
  }
  return {
    passed_declared_diversity_rule: comparisons.every(
      (item) => item.distinct_enough,
    ),
    rule: {
      minimum_differences: minimum,
      minimum_structural_differences: structuralMinimum,
    },
    comparisons,
    limitation:
      "Studio heuristic on declarations. Requires independent review of actual renders.",
  };
}
...
export function parsePortfolioRecord(raw: unknown): PortfolioRecord {
  ...
  keys(
    root,
    ["design_key", "project_key", "brand_key", "recorded_at", "signature"],
    "portfolio record",
  );
  if (
    typeof root.recorded_at !== "string" ||
    !/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d\.\d{3}Z$/.test(root.recorded_at)
  ) {
  ...
  if (
    ...
    time.toISOString() !== root.recorded_at
  )
    fail("recorded_at: invalid timestamp");
  return {
...
export function parsePortfolioOptions(raw: unknown): PortfolioOptions {
  ...
  for (const key of Object.keys(root)) {
    if (!["limit", "near_repeat_threshold", "ignore_same_brand"].includes(key))
      fail(`portfolio options: unknown field ${key}`);
  }
  const result: PortfolioOptions = {};
  if (root.limit !== undefined)
    result.limit = integer(root.limit, 1, 64, "limit");
  if (root.near_repeat_threshold !== undefined) {
    ...
    if (
      ...
      fail("near_repeat_threshold: expected 0 to 1");
    result.near_repeat_threshold = value;
  }
  if (root.ignore_same_brand !== undefined) {
    if (typeof root.ignore_same_brand !== "boolean")
      fail("ignore_same_brand: expected boolean");
...
export function assessPortfolio(
  ...
  if (!Array.isArray(rawHistory) || rawHistory.length > 10000)
    fail("history: expected an array with at most 10000 entries");
  ...
  const eligible = history
    .filter(
      (record) =>
        ...
        !(
          options.ignore_same_brand &&
          candidate.brand_key !== null &&
          record.brand_key === candidate.brand_key
        ),
    )
    .sort((a, b) => {
  ...
  const skipped: Array<{
    design_key: string;
    reason: "incompatible_or_invalid_signature";
  }> = [];
  const comparisons: Array<
    Comparison & { design_key: string; repeat_warning: boolean }
  > = [];
  for (const record of selected) {
    let prior: DesignSignature;
    try {
      prior = parseSignature(record.signature, catalog);
    } catch {
      skipped.push({
    ...
    comparisons.push({
      ...
      ...comparison,
      repeat_warning:
        comparison.declared_similarity >= threshold ||
        comparison.same_composition_and_order,
    });
  }
  return {
    ...
    comparison_coverage:
      selected.length === 0 ? null : comparisons.length / selected.length,
    skipped_history: skipped,
    warnings: comparisons.filter((item) => item.repeat_warning),
    comparisons,
    rule: {
      limit,
      near_repeat_threshold: threshold,
      ignore_same_brand: options.ignore_same_brand ?? false,
    },
    preserves_existing_gate:
      "Caller MUST retain current hard repetition validation. This helper emits advisory results only.",
    limitation:
