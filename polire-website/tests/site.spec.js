...
for (const [width,height] of [[1440,900],[1280,800],[1024,768],[768,1024],[390,844],[375,812]]) {
  test(`${width}px: navigation, language cycles, section visibility, layout`,async({page})=>{
    ...
    for (const language of ['en','de','en','de']) {
      ...
      expect(await page.locator('h1,h2,h3,button,.button,.nav').evaluateAll(nodes=>nodes.filter(n=>getComputedStyle(n).display!=='none'&&n.scrollWidth>n.clientWidth+2).map(n=>({tag:n.tagName,classes:n.className,text:n.textContent.slice(0,80),width:n.clientWidth,scroll:n.scrollWidth})))) .toEqual([]);
    }
...
test('keyboard disclosure, menu escape, legal links and email destination',async({page})=>{
  ...
  await expect(page.locator('footer a[href="mailto:hello@polire.ch"]')).toBeVisible();
