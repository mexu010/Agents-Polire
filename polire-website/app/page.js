...
import Portfolio, { FAQ } from "./portfolio";
...
import Deliverables, {RefinedMotion} from "./refinements";
...
export default function Home() {
  ...
  return <div className={`site-shell${intro ? " is-intro" : ""}`} ref={root}>
    ...
    <RefinedMotion/><main>{intro && <button className="intro-skip" onClick={skipIntro}>{t.motion.skipIntro}</button>}
      <section id="home" data-nav-section tabIndex={-1} className="hero wrap" aria-labelledby="hero-title"><HeroSculpture visual={t.heroVisual} motion={t.motion}/><div className="hero-copy"><p className="eyebrow">{t.eyebrow}</p><h1 id="hero-title" className="hero-title">{hero.title.map((line,index)=><span key={index} className={index===1?"hero-accent":undefined}><span aria-label={line}>{line.split(" ").map((word,i)=><span className="hero-word" key={i} aria-hidden="true" style={{"--word":i}}>{word}{" "}</span>)}</span></span>)}</h1><p className="hero-text">{hero.text}</p><div className="hero-actions"><a className="button button-dark magnetic" href="#contact">{hero.primary}</a><a className="text-link" href="#work">{hero.secondary}</a></div></div><div className="hero-foot"><span>{t.ui.studioNote}</span><span>{t.ui.scroll} </span></div></section>
      <section id="work" data-nav-section tabIndex={-1} className="work section wrap" aria-labelledby="work-title"><div className="section-heading"><p className="section-label">01 / {t.workKicker}</p><h2 id="work-title">{t.workTitle}</h2><p>{t.workIntro}</p></div><KineticStory motion={t.motion}/><Portfolio lang={lang}/></section>
      <Deliverables lang={lang}/><section id="services" data-nav-section tabIndex={-1} className="services section" style={{"--service-tone":Number(service||1)}} aria-labelledby="services-title"><div className="wrap services-layout"><div className="services-intro"><p className="section-label">02 / {t.servicesKicker}</p><h2 id="services-title">{t.servicesTitle}</h2><p>{t.servicesIntro}</p><div className="service-number" aria-hidden="true">{service||"—"}<span> / 05</span></div></div><div className="service-list"><div className="service-sculpture" aria-hidden="true">{Array.from({length:5},(_,i)=><i key={i} style={{"--blade":i}}/>)}</div>{t.services.map(([id,title,text])=><div className="service-item" key={id}><h3><button className="service-row" type="button" aria-expanded={service===id} aria-controls={`service-${id}`} onClick={()=>setService(window.innerWidth>860?id:service===id?null:id)}><span className="service-index">{id}</span><span>{title}</span><span className="service-plus" aria-hidden="true">{service===id?"−":"+"}</span></button></h3><div id={`service-${id}`} className="service-detail" hidden={service!==id}><p>{text}</p></div></div>)}</div></div></section>
      <section id="process" data-nav-section tabIndex={-1} className="process section wrap" aria-labelledby="process-title"><div className="section-heading"><p className="section-label">03 / {t.processKicker}</p><h2 id="process-title">{t.processTitle}</h2><p>{t.processIntro}</p></div><ol className="process-list">{t.process.map(([id,title,text])=><li key={id}><span className="process-index">{id}</span><h3>{title}</h3><p>{text}</p></li>)}</ol><div className="project-timeline"><h3>{t.timelineTitle}</h3><p>{t.timelineText}</p></div><div className="studio-note"><span className="section-label">{t.introKicker}</span><h3>{t.introTitle}</h3><p>{t.introText}</p></div></section>
      <FAQ lang={lang}/><section id="contact" data-nav-section tabIndex={-1} className="contact section" aria-labelledby="contact-title"><div className="wrap contact-inner"><p className="section-label">{t.ctaKicker}</p><h2 id="contact-title">{t.ctaTitle}</h2><div className="contact-bottom"><p>{t.ctaText}</p><div><ProjectEnquiry lang={lang} label={t.ctaButton}/><small>{t.contactNote}</small></div></div></div></section>
